...
export function useAnimationOptimizer(
  ...
  const playAnimation = useCallback((name: string, fadeIn = true) => {
  ...
  const stopAnimation = useCallback((name: string, fadeOut = true) => {
  ...
  const stopAllAnimations = useCallback((fadeOut = true) => {
    if (!actions) return;
