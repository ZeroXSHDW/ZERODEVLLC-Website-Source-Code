...
export const AnimatedButton = forwardRef<HTMLButtonElement, AnimatedButtonProps>(
  ({ variant = 'primary', size = 'md', isLoading = false, glowEffect = false, className = '', disabled, children, ...props }, ref) => {
    ...
    return (
      <motion.div
        ...
        animate={glowEffect ? {
          boxShadow: [variantStyles.shadow, `${variantStyles.shadow}, 0 0 20px rgba(59, 130, 246, 0.8)`, variantStyles.shadow],
        } : {}}
        transition={{ boxShadow: glowEffect ? { duration: 2, repeat: Infinity, ease: "easeInOut" } : undefined }}
      >
        <button
          ref={ref}
          ...
          style={{ background: variantStyles.background, boxShadow: variantStyles.shadow }}
        ...
        >
          ...
          />
          <motion.div className="relative z-10 flex items-center justify-center gap-2" animate={{ opacity: isLoading ? 0.5 : 1 }}>
            {children}
          </motion.div>
        </button>
      </motion.div>
